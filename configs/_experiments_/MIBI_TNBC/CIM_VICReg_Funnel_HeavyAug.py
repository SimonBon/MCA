...
# ── Heavy augmentations ───────────────────────────────────────────────────────
# Stronger channel noise, higher drop prob, wider scale range
# Goal: force model to learn patient-invariant representations via augmentation
train_aug_strong = [
    dict(type='C_RandomFlip',              prob=0.5, horizontal=True, vertical=True),
    dict(type='C_RandomAffine',            angle=(0, 360), scale=(0.7, 1.3), shift=(0, 0), order=1),
    dict(type='C_RandomChannelShiftScale', scale=(0.8, 1.4), shift=(0, 0), clip=True),
...
work_dir = '/nobackup/lab_taschner-mandl/simongutwein/z_RUNS/MIBI_TNBC_CIM_VICReg_Funnel_HeavyAug'
