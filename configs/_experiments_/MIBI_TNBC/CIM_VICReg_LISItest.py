...
# ── Short schedule: 500 iters (AdamW, no LARS needed for quick test) ──────────
n_linear = 100
n_cosine = 400
...
param_scheduler = [
    ...
    dict(type='CosineAnnealingLR', T_max=n_cosine,    by_epoch=False, begin=n_linear, end=n_linear + n_cosine, eta_min=3e-5),
]

# ── Augmentations (standard 2-view) ───────────────────────────────────────────
_shared = [
...
train_pipeline = [
    dict(type='C_MultiView', n_views=[1, 1], transforms=[_shared, _shared]),
    dict(type='C_PackInputs'),
]

_base_.val_augmentation[0].size = 20
...
_base_.custom_hooks[0].epochs         = 2000
...
work_dir = '/nobackup/lab_taschner-mandl/simongutwein/z_RUNS/MIBI_TNBC_CIM_VICReg_LISItest'
